...
impl AnthropicClient {
    pub fn new(config: AnthropicConfig) -> Result<Self> {
        let http = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| WhatCodeError::llm("anthropic", e.to_string()))?;
        Ok(Self { config, http })
    }
    ...
    async fn post(&self, body: Value) -> Result<Value> {
        let key = self
            ...
            .api_key
            .clone()
            .ok_or_else(|| WhatCodeError::llm("anthropic", "не задан ANTHROPIC_API_KEY"))?;
            ...
            async move {
                ...
                if status == 200 {
                    let value: Value = resp
                        .json()
                        .await
                        .map_err(|e| (false, WhatCodeError::llm("anthropic", e.to_string())))?;
                    return Ok(value);
                }
                ...
                Err((
                    retryable,
                    WhatCodeError::llm("anthropic", format!("HTTP {status}: {text}")),
                ))
            }
        })
        .await
