...
impl OpenAiCompatClient {
    pub fn new(provider: &'static str, config: OpenAiCompatConfig) -> Result<Self> {
        let http = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| WhatCodeError::llm(provider, e.to_string()))?;
        Ok(Self {
            provider,
            config,
            http,
    ...
    async fn post(&self, body: Value) -> Result<Value> {
        let key = self
            ...
            .api_key
            .clone()
            .ok_or_else(|| WhatCodeError::llm(self.provider, "отсутствует API-ключ"))?;
            ...
            async move {
                ...
                if status == 200 {
                    let value: Value = resp
                        .json()
                        .await
                        .map_err(|e| (false, WhatCodeError::llm(provider, e.to_string())))?;
                    return Ok(value);
                }
                ...
                Err((
                    retryable,
                    WhatCodeError::llm(provider, format!("HTTP {status}: {text}")),
                ))
            }
        })
        .await
...
impl ChatClient for OpenAiCompatClient {
    ...
    async fn warm_up(&self) -> Result<()> {
        if self.config.api_key.is_none() {
            return Err(WhatCodeError::llm(self.provider, "не задан API-ключ"));
        }
        Ok(())
    }
