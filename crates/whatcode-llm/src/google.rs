...
impl GoogleAiClient {
    pub fn new(config: GoogleAiConfig) -> Result<Self> {
        let http = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| WhatCodeError::llm("google_ai", e.to_string()))?;
        Ok(Self { config, http })
    }
    ...
    async fn generate(&self, model: &str, messages: &[Message]) -> Result<String> {
        let key = self
            ...
            .api_key
            .clone()
            .ok_or_else(|| WhatCodeError::llm("google_ai", "не задан API-ключ"))?;
            ...
            async move {
                ...
                if status == 200 {
                    let v: Value = resp
                        .json()
                        .await
                        .map_err(|e| (false, WhatCodeError::llm("google_ai", e.to_string())))?;
                    return Ok(v);
                }
                ...
                Err((
                    retryable,
                    WhatCodeError::llm("google_ai", format!("HTTP {status}: {text}")),
                ))
            }
        })
        .await?;
...
impl ChatClient for GoogleAiClient {
    ...
    async fn warm_up(&self) -> Result<()> {
        if self.config.api_key.is_none() {
            return Err(WhatCodeError::llm("google_ai", "не задан API-ключ"));
        }
        Ok(())
    }
